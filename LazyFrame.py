...
class LazyFrame:
  ...
  def _scan_python_function(
    ...
    *,
    ...
    if isinstance(schema, Mapping):
      self._ldf = PyLazyFrame.scan_from_python_function_pl_schema(list(schema.items()), scan_fn, pyarrow)
    else:
      self._ldf = PyLazyFrame.scan_from_python_function_arrow_schema(list(schema), scan_fn, pyarrow)
    return self
  ...
  def __getitem__(self, item: int | range | slice) -> LazyFrame:
    if not isinstance(item, slice):
      msg = "'LazyFrame' object is not subscriptable (aside from slicing)" "\n\nUse `select()` or `filter()` instead."
      raise TypeError(msg)
...
"""
  ...
  def _repr_html_(self) -> str:
    try:
      ...
      svg = subprocess.check_output(["dot", "-Nshape=box", "-Tsvg"], input=f"{dot}".encode())
      return "<h4>NAIVE QUERY PLAN</h4><p>run <b>LazyFrame.show_graph()</b> to see" f" the optimized version</p>{svg.decode()}"
    except Exception:
...
"""
  ...
  def describe(
    ...
    * We do not guarantee the output of `describe` to be stable. It will show
    ...
    for c, dtype in schema.items():
      ...
      # mean
      mean_expr = F.col(c).mean() if is_temporal or is_numeric or dtype == Boolean else null
      ...
      for p in quantiles:
        if is_numeric or is_temporal:
          pct_expr = F.col(c).to_physical().quantile(p, interpolation).cast(dtype) if is_temporal else F.col(c).quantile(p, interpolation)
          sort_cols.add(c)
        else:
          pct_expr = null
    ...
    column_metrics = [df_metrics.row(0)[(n * n_metrics) : (n + 1) * n_metrics] for n in range(schema.len())]
    summary = dict(zip(schema, column_metrics))

    # cast by column type (numeric/bool -> float), (other -> string)
    for c in schema:
      summary[c] = [  # type: ignore[assignment]
        (None if (v is None or isinstance(v, dict)) else (float(v) if (c in has_numeric_result) else str(v))) for v in summary[c]
      ]
  ...
  def explain(
    ...
    *,
    ...
    if tree_format is not None:
      issue_deprecation_warning(
        "The `tree_format` parameter for `LazyFrame.explain` is deprecated" " Use the `format` parameter instead.",
        version="0.20.30",
      )
      if tree_format:
        format = "tree"
  ...
  def show_graph(
    ...
    *,
    ...
    try:
      graph = subprocess.check_output(["dot", "-Nshape=box", "-T" + output_type], input=f"{dot}".encode())
    except (ImportError, FileNotFoundError):
      msg = "The graphviz `dot` binary should be on your PATH." "(If not installed you can download here: https://graphviz.org/download/)"
  ...
  def inspect(self, fmt: str = "{}") -> LazyFrame:
    ...
    return self.map_batches(inspect, predicate_pushdown=True, projection_pushdown=True)
  ...
  def sort(
    ...
    *more_by
    ...
    if isinstance(by, str) and not more_by:
      return self._from_pyldf(self._ldf.sort(by, descending, nulls_last, maintain_order, multithreaded))
    ...
    return self._from_pyldf(self._ldf.sort_by_exprs(by, descending, nulls_last, maintain_order, multithreaded))
  ...
  def sql(self, query: str, *, table_name: str = "self") -> LazyFrame:
    ...
    * More control over registration and execution behaviour is available by
    ...
    issue_unstable_warning("`sql` is considered **unstable** (although it is close to being considered stable).")
    ...
    *,
    ...
    if show_plot:
      ...
      if max_val > 1e9:
        ...
        unit = "us"
      if truncate_nodes > 0:
        timings_ = timings_.with_columns(F.col("node").str.slice(0, truncate_nodes) + "...")
  ...
  def collect(
    ...
    **_kwargs: Any,
    ...
    if (streaming or background or new_streaming) and is_gpu:
      issue_warning(
        "GPU engine does not support streaming or background collection, " "disabling GPU engine.",
  ...
  def cast(
    ...
    *,
    ...
    for c, dtype in dtypes.items():
      if (is_polars_dtype(c) or isinstance(c, DataTypeGroup)) or (isinstance(c, Collection) and all(is_polars_dtype(x) for x in c)):
        c = by_dtype(c)  # type: ignore[arg-type]

      dtype = parse_into_dtype(dtype)
      cast_map.update(
        {c: dtype} if isinstance(c, str) else {x: dtype for x in expand_selector(self, c)}  # type: ignore[arg-type]
      )
  ...
  def filter(
    ...
    **constraints: Any,
    ...
    for p in predicates:
      ...
      elif ((is_seq := is_sequence(p)) and any(not isinstance(x, pl.Expr) for x in p)) or (
        not is_seq and not isinstance(p, pl.Expr) and not (isinstance(p, str) and p in self.collect_schema())
      ):
        err = f"Series(…, dtype={p.dtype})" if isinstance(p, pl.Series) else repr(p)
        msg = f"invalid predicate for `filter`: {err}"
        raise TypeError(msg)
      else:
        all_predicates.extend(wrap_expr(x) for x in parse_into_list_of_expressions(p))
    ...
    combined_predicate = (F.all_horizontal(*all_predicates) if len(all_predicates) > 1 else all_predicates[0])._pyexpr if all_predicates else None

    # apply reduced boolean mask first, if applicable, then predicates
    ldf = self._ldf.filter(F.lit(reduce(and_, boolean_masks))._pyexpr) if boolean_masks else self._ldf
    return self._from_pyldf(ldf if combined_predicate is None else ldf.filter(combined_predicate))
  ...
  def select(self, *exprs: IntoExpr | Iterable[IntoExpr], **named_exprs: IntoExpr) -> LazyFrame:
    ...
    **named_exprs
    ...
    pyexprs = parse_into_list_of_expressions(*exprs, **named_exprs, __structify=structify)
  ...
  def rolling(
    ...
    *,
    ...
    pyexprs_by = parse_into_list_of_expressions(group_by) if group_by is not None else []
  ...
  def unpivot(
    ...
    *,
    ...
    if not streamable:
      issue_deprecation_warning(
        "The `streamable` parameter for `LazyFrame.unpivot` is deprecated" "This parameter has no effect",
        version="1.5.0",
      )
    ...
    result = (
      ...
      .with_columns(
        (
          ...
          F.when(F.col(validity).is_null()).then(F.col(name)).otherwise(F.col(f"{name}{tmp_name}"))
          if include_nulls
...
if TYPE_CHECKING:
  from polar_patch.aggrid import AggridFrame
